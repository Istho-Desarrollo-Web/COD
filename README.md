...
Los módulos de Usuarios (`/usuarios`), Roles de solo lectura (`/roles`) y Matriz de accesos de solo lectura (`/roles/matriz-accesos`) ya están implementados. Crear un Área acepta opcionalmente `liderUsuarioId` (usuario existente) o `nuevoUsuario` (crea el usuario y el área en una sola transacción). Un usuario puede tener varios roles simultáneos (relación N:M `Usuario↔Rol`); el área del usuario es un campo separado (`Usuario.areaId`), no parte del nombre del rol.
...
El módulo de Proveedores y Contratistas (`/proveedores`) ya está implementado: listado con filtros (estado, tipo, criticidad), creación (con selección del área solicitante), y detalle (`/proveedores/:id`) con edición inline (permiso `proveedores:gestionar`), baja lógica, y expediente documental — un checklist de los requisitos aplicables según la criticidad del proveedor (`critico`/`relevante`/`basico`: Cámara de Comercio, RUT, Certificado SST, Certificado SARLAFT, Póliza de responsabilidad civil), y subida/descarga/eliminación de los documentos que los cubren, con cálculo automático de vigencia (vigente/por vencer/vencido, umbral fijo de 30 días). La aprobación es en dos pasos, gateados por el permiso `proveedores:aprobar` (separado de `proveedores:gestionar`): **Aprobar registro** (`en_evaluacion` → `registro_aprobado`, solo valida el área solicitante) y **Aprobar requisitos** (`registro_aprobado` → `activo`, valida que el checklist de requisitos obligatorios esté completo, crea la carpeta del proveedor en Documentos y refleja el expediente). **Rechazar** (con motivo) es válido desde cualquiera de los dos estados intermedios.

`Administración > Logs del servidor` (`/administracion/logs`, permiso `logs_servidor:ver`) muestra el tráfico HTTP y los errores no controlados del backend, paginado y filtrable por nivel (info/warn/error), método, rango de fechas y texto libre. Los registros se purgan automáticamente a los 14 días (`npm run job:purgar-logs` para forzarlo manualmente).

`Administración > Matriz de accesos` (`/administracion/matriz-accesos`, permiso `matriz_accesos:ver`) muestra, de solo lectura, qué acciones tiene concedidas cada rol por módulo. Los permisos son globales por rol — el área de cada usuario se consulta por separado, en Administración > Usuarios.

**Limitación conocida:** el nombre del líder solo se resuelve para usuarios con permiso `usuarios:ver` — `GET /usuarios/:id` está gateado por ese permiso, y algunos roles con acceso a `/areas/:id` (`areas:ver`) no lo tienen. Para esos roles, la sección de líder muestra "Sin líder asignado" aunque sí haya uno asignado. Fast-follow pendiente: resolver el nombre del líder directamente en `GET /areas/:id` (backend) en vez de depender de `usuarioService.obtener`.
